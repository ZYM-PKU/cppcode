...
unsigned int chooseKeyPort=3;//波形选择

unsigned int ampGain=1;//幅度增益
unsigned int choice=0;//幅度增益
...
void setup() {  
  ...
  pinMode(chooseKeyPort,INPUT_PULLUP);
  freqStep = 1;          
...
void loop() {
  ...
  if (digitalRead(chooseKeyPort)==LOW){
      choice+=1;
      delay(300);
  }
  ...
  if(choice ==SIN)analogWrite(signalOutPWM, sinData[i]*ampGain/4);
  else if(choice ==SQUARE)analogWrite(signalOutPWM, squareData[i]*ampGain/4);
  else analogWrite(signalOutPWM, triangularData[i]*ampGain/4);
